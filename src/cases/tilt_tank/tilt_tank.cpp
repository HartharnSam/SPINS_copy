/* cases/tilt_tank/tilt_tank.cpp */
/* Early script for the formation of a convective plume
 * tilt, no boundary forcing */
...
double h_pyc;                   // density pycnocline width (m)
double z_pyc;                 // location of the center of the pycnocline (m)
    ...
    public:
        ...
        void init_tracers(vector<DTArray *> & tracers) {
            ...
            if (restarting and !restart_from_dump) {
                ...
                //rho += -0.5*delta_rho * tanh((zz(kk)-z_pyc)/h_pyc);
            } else if (restarting and restart_from_dump) {
        ...
        void forcing(double t, const DTArray & u, DTArray & u_f,
            ...
            u_f = +g*(*tracers[RHO])*sin(slope);
            v_f = 0;
            w_f = -g*(*tracers[RHO])*cos(slope);   // tracers[RHO] is defined as rho/rho_0
            *tracers_f[RHO] = 0;
        }
        ...
        void analysis(double time, DTArray & u, DTArray & v, DTArray & w,
            ...
            if ((time - next_plot) > -1e-6) {
                ...
                //*temp1 = +0.5*delta_rho*tanh((zz(kk)-z_pyc)/h_pyc) + (*tracers[RHO]);
...
int main(int argc, char ** argv) {
    ...
    add_option("h_pyc",&h_pyc,"density pycnocline width");
    add_option("z_pyc",&z_pyc,"location of the center of the pycnocline");
	...
	add_option("slope", &slope, "Slope of tank");
    ...
    add_option("compute_BPE",&compute_BPE,false,"Calculate BPE?");
    add_option("compute_internal_to_BPE",&compute_internal_to_BPE,false,
