/* cases/buoyant_plume_extension/buoyant_plume_extension.cpp */
...
class userControl : public BaseCase {
    public:
        ...
        void do_mapping(DTArray & xg, DTArray & yg, DTArray & zg) {
			fprintf(stdout, "Starting to sort some griddy things");
        ...
        void init_vels(DTArray & u, DTArray & v, DTArray & w) {
            ...
            if (restarting and !restart_from_dump) {
                ...
                if (Ny > 1 || rot_f != 0) {
                    if (v_filename.length() > 0) { // v-file specified
						fprintf(stdout,"Trying to read in a v file \n");
                        init_field("v", v_filename, v, input_data_types);
                    } else { // v meaningful but not given
                        if (master()) {
                            fprintf(stdout,"INFO: V-field meaningful but not given -- providing default initialization\n");
                        }
                        v = 0*ii + 0*jj + 0*kk;
						fprintf(stdout,"v array made \n");
                    }
                }
                ...
                if (Ny > 1 || rot_f != 0) {
					...
					fprintf(stdout,"v array written \n");
                }
            }
        }
		...
		void tracer_bc_z(int t_num, double & dir, double & neu) const {
			...
			if (t_num == RHO) {
				// density satisfies a Dirichlet-type boundary condition (fixed density)
				dir = 1.0; 
				neu = 0.0;
			} else {
				// Everything else satisfies a Neumann-type BC (no flux)
				dir = 0.0; 
				neu = 1.0;
			}
		}
        ...
        void forcing(double t, const DTArray & u, DTArray & u_f,
            ...
            w_f = -g*(*tracers[RHO])*cos(slope*M_PI/180)- rot_f*v*sin(slope*M_PI/180);   // tracers[RHO] is defined as rho/rho_0
			
            *tracers_f[RHO] = 0;
			(*tracers_f[RHO])(blitz::Range::all(),blitz::Range::all(), 0) = rho_bot;
			(*tracers_f[RHO])(blitz::Range::all(),blitz::Range::all(),Nz-1) = rho_top;
        }
...
int main(int argc, char ** argv) {
	...
	add_option("rho_bot",&rho_bot,"Bottom forcing density");
	add_option("rho_top",&rho_top,"Top forcing density");
	add_option("slope",&slope,"Slope");
		
    option_category("Second tracer");
    add_option("enable_tracer",&is_tracer,false,"Enable evolution of a second tracer");
    //add_option("tracer_kappa",&kappa_trc,0,"Diffusivity of tracer");
    ...
    userControl mycode;
	//fprintf(stdout,"MyCode created, what's next? \n");
    // Create a flow-evolver that takes its settings from the above class
    FluidEvolve<userControl> do_stuff(&mycode);
	//fprintf(stdout,"FluidEvolve created, what's next? \n");
    // Initialize
    do_stuff.initialize();
	//fprintf(stdout,"Initialised, what's next? \n");
    ...
    if (master()) {
        fprintf(stdout,"3-D Extension of Buoyant Plume problem\n");
