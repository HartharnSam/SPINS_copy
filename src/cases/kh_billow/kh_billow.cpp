...
//		z_locs = 0.1, 0.15, 0.2;         // Arrays and operators for derivatives
        Grad * gradient_op;
		...
		int z_loc_size = 3;
		blitz::Array<double,1> z_locs(z_loc_size);
		z_locs(0) = 0.1; 
		z_locs(1) = 0.15;
		z_locs(2) = 0.2;
        ...
        void analysis(double time, DTArray & u, DTArray & v, DTArray & w,
			...
			if ( compute_point_ts ) {
				point_ts(*tracers[RHO], xx, zz, z_locs, x_loc, time, iter, restarting);
			}
