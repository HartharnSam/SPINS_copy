 ...
 * and with "topography" at the surface */
...
double pyc_loc;                 // pycnocline location (m)
double h_halfwidth;             // pycnocline half-width (m)
double pyc_adj_loc;             // pycnocline location in lock-release (adjusted) region (m)
double h_adj_halfwidth;         // pycnocline half-width in adjusted region (m)
...
double ice_thickness;			// thickness of surface block (m)
double ice_trans;              	// transition length from uncovered to full thickness region (m)
double ice_length;              // location from end of tank to end of surface block (m)
...
class userControl : public BaseCase {
    public:
        ...
        void do_mapping(DTArray & xg, DTArray & yg, DTArray & zg) {
            ...
            topo = -(ice_thickness/2 + (ice_thickness*0.5*tanh((xx(ii)+ice_length-Lx)/ice_trans)));
        ...
        void init_tracers(vector<DTArray *> & tracers) {
            ...
            if (restarting and !restart_from_dump) {
                ...
                rho =  -0.5*delta_rho*tanh(((*zgrid)(ii,jj,kk)-pyc_loc)/h_halfwidth);
                rho = rho*0.5*(1.0+tanh((xx(ii)-L_adj)/delta_x));
                // mixed region
                rho += 0.5*(1.0-tanh((xx(ii)-L_adj)/delta_x))
                    *(-0.5*delta_rho*tanh(((*zgrid)(ii,jj,kk)-pyc_adj_loc)/h_adj_halfwidth));
...
int main(int argc, char ** argv) {
    ...
    add_option("ice_length",&ice_length,"Height of hill");
    add_option("ice_thickness",&ice_thickness,"Slope of hill");
    add_option("ice_trans",&ice_trans,"Hill adjustment parameter");
    ...
    add_option("pyc_adj_loc",&pyc_adj_loc,"pycnocline location");
    add_option("h_adj_halfwidth",&h_adj_halfwidth,"Pycnocline half-width");
