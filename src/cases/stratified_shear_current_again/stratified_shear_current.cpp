/* cases/buoyant_plume/buoyant_plume.cpp */
/* Script for buoyant plume forced by surface conditions :
 * zero initial velocity (with white noise)
 * density based
 * Initial condition thin stratified layer at surface 
 * Density forcing at the upper and lower boundary at fixed density 
 * Rotation optional implemented for tilt
 */
...
string grid_type[3];
bool rho_dir_bc; 				// is the density vertical BC dirichlet (default false)
// Physical parameters
...
const int RHO = 0;
// Problem parameters
double rho_bot;                  // Temperature at the bottom boundary of the domain (Deg C)
double rho_top;                  // Temperature at the top boundary of the domain (Deg C)      
double h_pyc;                    // Thickness of thin bounary layer at surface (m) 
double pyc_perc; 				 // Percentage of total depth the pycnocline sits at [0-1]
double delta_u;					 // Initial horizontal velocity difference
...
double slope;                   // slope angle in degrees
// Temporal parameters
    ...
    public:
        ...
        void init_vels(DTArray & u, DTArray & v, DTArray & w) {
            ...
            if (restarting and !restart_from_dump) {
				...
				if ( pyc_perc == 1 ) {
					u = 0.5*delta_u*tanh((zz(kk)-(Lz-h_pyc))/(h_pyc/2));
				} else {
					u = 2*delta_u*(-.5+pow(sech((zz(kk)-(Lz*(1-pyc_perc)))/(h_pyc/2)), 2));
				}
                v = 0;
                w = 0;
      ...
      void tracer_bc_z(int t_num, double & dir, double & neu) const {
        ...
        void init_tracers(vector<DTArray *> & tracers) {
            ...
            if (restarting and !restart_from_dump) {
				...
				if ( pyc_perc == 1 ) {
					rho = rho_top+(0.5*(rho_bot-rho_top)*(1-tanh((zz(kk)-(Lz-h_pyc))/(h_pyc/2))));
				} else {
					rho = rho_top+(0.5*(rho_bot-rho_top)*(1-tanh((zz(kk)-(Lz*(1-pyc_perc)))/(h_pyc/2))));
				}
        ...
        void forcing(double t, const DTArray & u, DTArray & u_f,
            ...
            //u_f = (+rot_f*v)
			u_f = +g*(*tracers[RHO])*sin(slope*M_PI/180) + (+rot_f*v*cos(slope*M_PI/180));
            v_f = -rot_f*(u*cos(slope*M_PI/180) - w*sin(slope*M_PI/180));
            w_f = -g*(*tracers[RHO])*cos(slope*M_PI/180) - rot_f*v*sin(slope*M_PI/180);
            
            *tracers_f[RHO] = 0;
            /* Impose tracer boundary conditions */
			if ( rho_dir_bc ) {
				(*tracers_f[RHO])(blitz::Range::all(),blitz::Range::all(),0) = rho_bot; // BC at k=0
				(*tracers_f[RHO])(blitz::Range::all(),blitz::Range::all(),Nz-1) = rho_top; // BC at k=max
			}
        }
        ...
        void analysis(double time, DTArray & u, DTArray & v, DTArray & w,
            ...
            if (compute_internal_to_BPE) {
                compute_BPE_from_internal(phi_i, *tracers[RHO], kappa, rho_0, g, Nz);
            }
            // viscous dissipation
            double diss_tot = 0;
            double max_diss = 0;
            ...
            double mass = pssum(sum(rho_0*((*tracers[RHO]))*(*dxdydz)));
...
int main(int argc, char ** argv) {
	...
	add_option("rho_dir_bc",&rho_dir_bc,false,"Is the density z BC Dirichlet?");
    ...
    add_option("slope",&slope,"Slope of tank in degrees");
    ...
    add_option("rho_bot",&rho_bot,"Initial bottom temperature");
    add_option("rho_top",&rho_top,"Initial top temperature");
    add_option("h_pyc",&h_pyc,"Pycnocline thcikness");
	add_option("pyc_perc",&pyc_perc,"Percentage of total depth the pycnocline sits at [0-1]");
	add_option("delta_u",&delta_u,"Initial horizontal velocity difference");
    ...
    if (master()) {
        fprintf(stdout,"Buoyant Plume problem\n");
