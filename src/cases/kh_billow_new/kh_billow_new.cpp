...
double R;                 		// Interface Width ratio 
double a;						// Interface offset
    ...
    public:
        ...
        void init_vels(DTArray & u, DTArray & v, DTArray & w) {
            ...
            if (restarting and !restart_from_dump) {
                ...
                u = 0.5*delta_u*tanh(2*zz(kk)/dz_u);
                v = 0;
                w = 0;
        ...
        void init_tracers(vector<DTArray *> & tracers) {
            ...
            if (restarting and !restart_from_dump) {
                ...
                rho += -0.5*delta_rho * tanh((2*R*zz(kk)/dz_rho)+a);
            } else if (restarting and restart_from_dump) {
                ...
                rho = rho_perturb * delta_rho *
                    cos(perturb_k*xx(ii)) * pow(cosh((2*R*zz(kk)/dz_rho)+a),-2);
                // Write the array
        ...
        void analysis(double time, DTArray & u, DTArray & v, DTArray & w,
            ...
            if ((time - next_plot) > -1e-6) {
                ...
                *temp1 = +0.5*delta_rho*tanh((2*R*zz(kk)/dz_rho)+a) + (*tracers[RHO]);
...
int main(int argc, char ** argv) {
    ...
    add_option("a",&a,"Offset of interfaces");
    add_option("dz_rho",&dz_rho,"density pycnocline width");
    add_option("R",&R,"Interface thickness ratio");
